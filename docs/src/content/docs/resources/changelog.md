...
- Added `tag-base`, `tag-content`, and `tag-remove-button` parts to `<wa-select>` [#682]
...
- Fixed a bug where pressing up/down in `<wa-tab-group>` didn't select the next/previous tab in vertical placements
...
- Fixed a bug in `<wa-button>` and `<wa-icon-button>` where an unintended `ref` attribute was present
...
- 🚨 BREAKING: `<wa-radio>` components must be located inside an `<wa-radio-group>` for proper accessibility [#218]
...
- Fixed a bug where the check icon was announced to screen readers in `<wa-menu-item>`
- Improved `<wa-icon-button>` accessibility by encouraging proper use of `label` and hiding the internal icon from screen readers [#220]
- Improved `<wa-dropdown>` accessibility by attaching `aria-haspopup` and `aria-expanded` to the slotted trigger
- Refactored position logic to remove an unnecessary state variable in `<wa-image-comparer>`
...
- Added `label` slot to `<wa-input>`, `<wa-select>`, and `<wa-textarea>` [#248]
- Added `label` slot to `<wa-dialog>` and `<wa-drawer>`
- Added experimental `<wa-include>` component
- Added status code to the `wa-error` event in `<wa-icon>`
- Fixed a bug where initial transitions didn't show in `<wa-dialog>` and `<wa-drawer>` [#247]
...
- 🚨 BREAKING: Transformed all Web Awesome events to lowercase ([details](#why-did-event-names-change))
- Added support for dropdowns and non-icon elements to `<wa-input>`
- Added `spellcheck` attribute to `<wa-input>`
- Added `<wa-icon-library>` to allow custom icon library registration
- Added `library` attribute to `<wa-icon>` and `<wa-icon-button>`
- Added "Integrating with Rails" tutorial to the docs, courtesy of [ParamagicDev](https://github.com/ParamagicDev)
- Fixed a bug where `<wa-progress-ring>` rendered incorrectly when zoomed in Safari [#227]
- Fixed a bug where tabbing into slotted elements closes `<wa-dropdown>` when used in a shadow root [#223]
- Fixed a bug where scroll anchoring caused undesirable scrolling when `<wa-details>` are grouped

Web Awesome events were updated to use a lowercase, kebab-style naming convention. Instead of event names such as `slChange` and `slAfterShow`, you'll need to use `wa-change` and `wa-after-show` now.

This change was necessary to address a critical issue in frameworks that use DOM templates with declarative event bindings such as `<wa-button @slChange="handler">`. Due to HTML's case-insensitivity, browsers translate attribute names to lowercase, turning `@slChange` into `@slchange`, making it impossible to listen to `slChange`.

While declarative event binding is a non-standard feature, not supporting it would make Web Awesome much harder to use in popular frameworks. To accommodate those users and provide a better developer experience, we decided to change the naming convention while Web Awesome is still in beta.
...
- Refactored clear logic in `<wa-input>`

## 2.0.0-beta.18

- Added `name` and `invalid` attribute to `<wa-color-picker>`
- Added support for form submission and validation to `<wa-color-picker>`
- Added touch support to demo resizers in the docs
- Added `<wa-responsive-embed>` component
- Fixed a bug where swapping an animated element wouldn't restart the animation in `<wa-animation>`
- Fixed a bug where the cursor was incorrect when `<wa-select>` was disabled
- Fixed a bug where `slblur` and `slfocus` were emitted twice in `<wa-select>`
- Fixed a bug where clicking on `<wa-menu>` wouldn't focus it
- Fixed a bug in the popover utility where `onAfterShow` would fire too soon
- Fixed a bug where `bottom` and `right` placements didn't render properly in `<wa-tab-group>`
- Improved keyboard logic in `<wa-dropdown>`, `<wa-menu>`, and `<wa-select>`
- Updated `<wa-animation>` to stable
- Updated to Stencil 2.0 (you may need to purge `node_modules` and run `npm install` after pulling)
- Updated entry points in `package.json` to reflect new filenames generated by Stencil 2

## 2.0.0-beta.17

- Added `minlength` and `spellcheck` attributes to `<wa-textarea>`
- Fixed a bug where clicking a tag in `<wa-select>` wouldn't toggle the menu
- Fixed a bug where options where `<wa-select>` options weren't always visible or scrollable
- Fixed a bug where setting `null` on `<wa-input>`, `<wa-textarea>`, or `<wa-select>` would throw an error
- Fixed a bug where `role` was on the wrong element and aria attribute weren't explicit in `<wa-checkbox>`, `<wa-switch>`, and `<wa-radio>`
- Fixed a bug where dynamically adding/removing a slot wouldn't work as expected in `<wa-card>`, `<wa-dialog>`, and `<wa-drawer>`
- Fixed a bug where the value wasn't updated and events weren't emitted when using `setRangeText` in `<wa-input>` and `<wa-textarea>`
...
- Added `required` to `<wa-checkbox>`, `<wa-select>`, and `<wa-switch>`
- Added `reportValidity()` and `setCustomValidity()` methods to all form controls
- Added validation checking for custom and native form controls to `<wa-form>`
- Added `novalidate` attribute to `<wa-form>` to disable validation
...
- Added `hoist` attribute to `<wa-color-picker>`, `<wa-dropdown>`, and `<wa-select>` to work around panel clipping
- Added `<wa-format-bytes>` utility component
- Added `clearable` and `required` props to `<wa-select>`
- Added `slclear` event to `<wa-input>`
- Added keyboard support to the preview resizer in the docs
- Fixed a bug where the `aria-selected` state was incorrect in `<wa-menu-item>`
- Fixed a bug where custom properties applied to `<wa-tooltip>` didn't affect show/hide transitions
- Fixed a bug where `--wa-input-color-*` custom properties had no effect on `<wa-input>` and `<wa-textarea>`
- Refactored `<wa-dropdown>` and `<wa-tooltip>` to use positioner elements so panels/tooltips can be customized easier

## 2.0.0-beta.15

- Added `image-comparer` component
- Added `--width`, `--height`, and `--thumb-size` custom props to `<wa-switch>`
- Fixed an `aria-labelledby` attribute typo in a number of components
- Fixed a bug where the `change` event wasn't updating the value in `<wa-input>`
- Fixed a bug where `<wa-color-picker>` had the wrong border color in the dark theme
- Fixed a bug where `<wa-menu-item>` had the wrong color in dark mode when disabled
- Fixed a bug where WebKit's autocomplete styles made inputs looks broken
- Fixed a bug where aria labels were wrong in `<wa-select>`
- Fixed a bug where clicking the label wouldn't focus the control in `<wa-select>`

## 2.0.0-beta.14

- Added dark theme
- Added `--sl-panel-background-color` and `--sl-panel-border-color` tokens
- Added `--tabs-border-color` custom property to `<wa-tab-group>`
- Added `--track-color` custom property to `<wa-range>`
- Added `tag` part to `<wa-select>`
...
- Fixed a bug where the text color was being erroneously set in `<wa-progress-ring>`
- Fixed a bug where `<wa-progress-bar>` used the wrong part name internally for `indicator`
- Removed background color from `<wa-menu>`
- Updated to Stencil 1.17.3

## 2.0.0-beta.13

- Added `slactivate` and `sldeactivate` events to `<wa-menu-item>`
- Added experimental `<wa-animation>` component
...
- Moved scrolling logic from `<wa-menu>` to `<wa-dropdown>`

## 2.0.0-beta.12

- Added support for `href`, `target`, and `download` to buttons
- Fixed a bug where buttons would have horizontal spacing in Safari
- Fixed a bug that caused an import resolution error when using Web Awesome in a Stencil app

## 2.0.0-beta.11

- Added button group component
- Fixed icon button alignment
- Fixed a bug where focus visible observer wasn't removed from `<wa-details>`
...
- Replaced make-shift icon buttons with `<wa-icon-button>` in alert, dialog, drawer, and tag
...
- Fixed a bug where nested form controls were ignored in `<wa-form>`
