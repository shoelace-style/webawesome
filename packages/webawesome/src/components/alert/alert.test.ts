/** biome-ignore-all lint/style/noNonNullAssertion: shadowRoot is open */
...
const expectHideAndAfterHideToBeEmittedInCorrectOrder = async (alert: WaAlert, action: () => void | Promise<void>) => {
  ...
  // biome-ignore lint/suspicious/noAssignInExpressions: ignore assignment in expression
...
const getCloseButton = (alert: WaAlert): WaButton | null | undefined =>
  alert.shadowRoot?.querySelector<WaButton>('[part="close-button"]');
  ...
  for (const fixture of [clientFixture]) {
    ...
    describe('timer controlled closing', () => {
      it('closes after a predefined amount of time', async () => {
        ...
        const alert = await fixture<WaAlert>(html` <wa-alert open duration="2000">I am an alert</wa-alert>`);
      ...
      it.skip('resets the closing timer after opening', async () => {
        clock = sinon.useFakeTimers();
        const alert = await fixture<WaAlert>(html` <wa-alert open duration="2000">I am an alert thing</wa-alert>`);
