...
function syncCheckboxes(changedTreeItem: WaTreeItem, initialSync = false) {
  ...
  function syncAncestors(treeItem: WaTreeItem) {
    const parentItem: WaTreeItem | null = treeItem.parentElement as WaTreeItem;
  ...
  function syncDescendants(treeItem: WaTreeItem) {
...
export default class WaTree extends WebAwesomeElement {
  ...
  private handleTreeChanged = (mutations: MutationRecord[]) => {
    for (const mutation of mutations) {
      const addedNodes: WaTreeItem[] = [...mutation.addedNodes].filter(WaTreeItem.isTreeItem) as WaTreeItem[];
      const removedNodes = [...mutation.removedNodes].filter(WaTreeItem.isTreeItem) as WaTreeItem[];
  ...
  private selectItem(selectedItem: WaTreeItem) {
  ...
  private getAllTreeItems() {
    return [...this.querySelectorAll<WaTreeItem>('wa-tree-item')];
  }

  private focusItem(item?: WaTreeItem | null) {
    item?.focus();
  }
  ...
  handleMouseDown(event: MouseEvent) {
    ...
    this.clickTarget = event.target as WaTreeItem;
  }
  ...
  get selectedItems(): WaTreeItem[] {
    const items = this.getAllTreeItems();
    const isSelected = (item: WaTreeItem) => item.selected;
  ...
  getFocusableItems() {
    ...
    return items.filter(item => {
      ...
      const parent: WaTreeItem | null | undefined = item.parentElement?.closest('[role=treeitem]');
