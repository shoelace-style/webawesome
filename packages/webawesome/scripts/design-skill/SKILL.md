...
description: >
  Design and lay out user interfaces with Web Awesome. Use when building or styling a page, layout, or
  section; choosing or customizing a theme; applying brand colors or design tokens; or composing a
  polished UI. Triggers on "build a landing page", "set up <wa-page>", "add a sidebar", "apply a theme",
  "match our brand color", "lay out a dashboard". Pair with the `webawesome` skill for component APIs.
license: MIT
metadata:
  author: Web Awesome
