...
Pickers are form controls that pair a **segmented input** with a **popup** for visual selection — `<wa-date-picker>` and `<wa-time-picker>` are the canonical examples. When building a new picker, follow these conventions so it composes cleanly with the rest of the library.
